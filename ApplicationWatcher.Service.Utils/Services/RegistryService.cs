...
namespace ApplicationWatcher.Service.Utils.Services
{
    public class RegistryService : IRegistryService
    {
        ...
        public T GetRegistryValue<T>(string basePath, string valueName)
        {
            T value = default;
            ...
            if (!string.IsNullOrEmpty(basePath) && !string.IsNullOrEmpty(valueName))
            {
                try
                {
                    value = _registryWrapperService.GetRegistryValue<T>(basePath, valueName, RegistryHive.LocalMachine);
                    if (value == null)
                    {
